...
my_theme1 <- theme_classic(base_size=25) + theme(axis.text.y=element_text(colour="black"), axis.text.x=element_text(colour="black"), axis.line=element_line(colour="black", size=1.7, lineend="square"))

my_theme2 <- theme_bw(base_size=25) + theme(axis.text.y=element_text(colour="black"), axis.text.x=element_text(colour="black"), axis.line=element_line(colour="black", size=1.7, lineend="square"))

my_theme3 <- theme_minimal(base_size=25) + theme(axis.text.y=element_text(colour="black"), axis.text.x=element_text(colour="black"), axis.line=element_line(colour="black", size=1.7, lineend="square"))
```
...
- もしRawDataが整理されていない場合には、データを加工したものをcsvファイルとして./Data/に別途保存し、それを読み込み直すのが良い。この場合には、別のRスクリプトで加工用ファイルを用意する方がきれいになる（Rmarkdownでやらない方が良い）。
...
```{r, code = readLines(here("Analysis_Script", "_functions", "plot_TrialQvalue.R"))}
```
...
```{r, code = readLines(here("Analysis_Script", "_functions", "plot_TrialProb.R"))}
```
