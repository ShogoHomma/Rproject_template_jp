...
rmd_dir <- "Analysis_Script"
cache_dir <- "_cache" # cacheの保存ディレクトリ名
rmd_input <- knitr::current_input() %>% str_split(pattern = "\\.Rmd") %>% .[[1]] %>% .[1] # 拡張子の前だけ取り出す # 返り値がリストになっているのでその中身を取り出す

knitr::opts_chunk$set(
  echo = TRUE, comment = "", warning = FALSE, message = FALSE, fig.align = "center",
  cache.path = paste0(here(rmd_dir, cache_dir, paste0(rmd_input, "_cache"), "html"), "/"), 
  fig.path = paste0(here(rmd_dir, cache_dir, paste0(rmd_input, "_files"), "figure-html"), "/")
)
```
...
# dir.create(output_path) # なければ、フォルダを作成
```
...
df_dummy_RL <- purrr::map_dfr(target_csv, ~readr::read_csv(.x, show_col_types = FALSE))
...
```{r}
target_file_path_params <- here("Data", "Models_Results", "basic_RL", "fit01", "estimated_parameter", "est_parameter.csv")

df_est_params <- 
  readr::read_csv(target_file_path_params, show_col_types = FALSE) %>% 
  dplyr::rename(
    ID = p_index
  ) %>% 
...
```{r}
# Dummyデータとパラメータ推定の2つを結合

target_file_path_data <- here("Data", "DummyData_RL", "DummyData_RL_Random_trialN200_1Op0.7_2Op0.3_agentN200_20210707181150", "RL_trial.csv")

target_file_path_params <- here("Data", "Models_Results", "basic_RL", "fit04", "estimated_parameter", "est_parameter.csv")

df_dummy_RL <- readr::read_csv(target_file_path_data, show_col_types = FALSE)

df_est_params <- readr::read_csv(target_file_path_params, show_col_types = FALSE) %>% 
  dplyr::rename(
    ID = p_index
  ) %>% 
  dplyr::filter(para %in% c("alpha", "beta")) %>% 
  tidyr::pivot_wider(id_cols = c("ID"), names_from = para, values_from = mode) %>% 
  dplyr::rename(
    fit_alpha = alpha,
    fit_beta = beta
  )
...
df_RL %>% 
  ggplot(aes(x = alpha, y = fit_alpha)) + geom_point() + 
  my_theme2

cor(df_RL$alpha, df_RL$fit_alpha)

df_RL %>% 
  ggplot(aes(x = beta, y = fit_beta)) + geom_point() + 
  my_theme2

cor(df_RL$beta, df_RL$fit_beta)
...
target_csv <- make_DataPath(DataDir, DataDir_sub, "", "_generation.csv")
df_PGG <- purrr::map_dfr(target_csv, readr::read_csv)
```


```{r}
df_PGG %>% 
  tidyr::pivot_longer(cols = -Generation, names_to = c("f", "Strategy"), values_to = "frequency", names_sep = "_") %>% 
  ggplot(aes(x = Generation, y = frequency, color = Strategy)) + geom_line()
```
